...
LLM strategy uses the configured `JsonMaterializer` to normalize them to
`JsonSnapshot` before constructing a transport-facing request.
...
provider-neutral `JsonSnapshot`. Configure custom inspection on a
