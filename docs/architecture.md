...
| `llm/` | The **default** `InferenceStrategy`: `step_decision.py` owns the provider-neutral step-decision model and validation, `structured_value.py` owns generic value-shape operations, `result_schema.py` defines result schema generation/restoration, `json_schema/` contains only JSON Schema concepts, and `_strategy.py` orchestrates calls and repair. | `LLMInferenceStrategy`, `LLMClient`, `StepDecisionSpec`, `StepDecisionModel`, `StructuredValue`, `ResultSchema`, `JsonSchemaDocument`, prompt formatters |
