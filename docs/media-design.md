...
| **Name the type `File` or `Attachment`.** | `File` names the storage mechanism (the `Resource` argument again) and much of what `Media` references is not a file — `https:` URLs, `data:` URIs, `media://` store entries; worse, it invites confusion with provider Files APIs, whose upload handles are exactly the provider-scoped state kept out of core. `Attachment` names a message-relative role (`Media` stands alone in signatures and history), and in MIME the word denotes the opposite disposition: `Content-Disposition: attachment` means *not displayed inline*, while rendering into context is the `inline` disposition. The `str`-vs-`Media` split axis is the semantic role — prompt text vs content opted-in for rendering — not physical inlining: a `data:` `Media` is inline bytes and still `Media`; the physical form (url / inline / store) is the URI's dimension. |
