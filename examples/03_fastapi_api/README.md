...
when the agent needs more detail, the `interaction_required` event finalizes that
...
bubble it belongs to; the discrete `interaction_required` / `output` events close that
bubble.
...
starts the workflow with an empty body; the normal HTTP response is
`interaction_required` with an `interaction_id` and the complete `request` payload. The same event is also published on
...
curl -s -X POST localhost:8000/sessions/$SID/interview \
  -H 'content-type: application/json' \
  -d '{"result": "An article about our new product for developers.", "interaction_id": "<interaction_id>"}'
```
...
events.addEventListener("interaction_required", (event) => console.log(JSON.parse(event.data)))
...
- Streaming parsed `delta` text (with a `type` of `input`/`output`) and publishing `completed`, `interaction_required`, and `execution_failed` events through a separate SSE channel
...
concurrently: SessionScope binds the interaction channel per task
