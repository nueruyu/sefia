...
@final
class PydanticStructuredDataConverter(StructuredDataConverter):
    """Converts Python runtime values to structured data with Pydantic support."""

    @override
    def to_structured_data(self, value: object) -> StructuredData:
        return _to_structured_data(value)


def _to_structured_data(value: object) -> StructuredData:
    if isinstance(value, Enum):
        return _to_structured_data(value.value)
    if value is None or isinstance(value, str | int | float | bool):
        return StructuredData.from_scalar(value)
    if isinstance(value, BaseModel):
        serialized = TypeAdapter(type(value)).dump_python(value, mode="json")
        return _to_structured_data(serialized)
    if is_dataclass(value) and not isinstance(value, type):
        return StructuredData.from_object(
            {
                field.name: _to_structured_data(getattr(value, field.name))
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in cast(Mapping[object, object], value).items():
            ...
            entries[structured_key] = _to_structured_data(item)
        return StructuredData.from_mapping(entries)
    if isinstance(value, list | tuple):
        sequence = cast(list[object] | tuple[object, ...], value)
        return StructuredData.from_array(_to_structured_data(item) for item in sequence)
    return StructuredData.from_scalar(str(value))


def _dump_key(value: object) -> JsonScalar:
    structured = _to_structured_data(value).tree
    if structured is None or isinstance(structured, str | int | float | bool):
        return structured
    return str(structured)


__all__ = ["PydanticStructuredDataConverter"]
