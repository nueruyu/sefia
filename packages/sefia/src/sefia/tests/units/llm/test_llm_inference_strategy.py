...
from sefia.llm.events import LLMReasoningTokenReceived, LLMTokenReceived
...
class TestLLMInferenceStrategy:
    ...
    async def test_decide_next_step_publishes_reasoning_tokens(self, mock_llm_client):
        ...
        assert any(
            isinstance(event, LLMReasoningTokenReceived)
            and event.token == "thinking"
            for event in publisher.events
        )
    ...
    async def test_decide_next_step_does_not_set_reasoning_callback_by_default(
        ...
        assert (
            mock_llm_client.complete.await_args.kwargs["reasoning_callback"] is None
        )
