...
class TestPydanticModelInspector:
    def test_get_type_schema_for_primitive_type(self):
    ...
    def test_get_type_schema_for_dataclass_type(self):
        inspector = PydanticModelInspector()
    ...
    def test_get_arguments_model_validates_and_coerces(self):
        inspector = PydanticModelInspector()

        model = inspector.get_arguments_model(_sample_func)
        validated = inspector.validate(model, {"a": "5"})

        assert validated.a == 5
        assert validated.b == "x"

    def test_get_arguments_model_rejects_unknown_argument(self):
        inspector = PydanticModelInspector()

        model = inspector.get_arguments_model(_sample_func)

        with pytest.raises(ValueError, match="Model validation failed"):
            inspector.validate(model, {"a": 1, "unexpected": True})

    def test_get_arguments_model_rejects_missing_required_argument(self):
        inspector = PydanticModelInspector()
    ...
    def test_get_arguments_model_is_cached(self):
        inspector = PydanticModelInspector()

        assert inspector.get_arguments_model(_sample_func) is (
            inspector.get_arguments_model(_sample_func)
        )
