...
def prepare_schema(schema: SchemaObject, policy: SchemaPolicy) -> PreparedSchema:
...
def _apply_corrections(schema: SchemaObject, policy: SchemaPolicy) -> None:
...
def _validate(schema: SchemaObject, constraints: SchemaConstraints) -> None:
    for cursor in SchemaNode(schema).walk():
