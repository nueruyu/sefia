...
These subclass :class:`sefia.exceptions.InferenceError` (and therefore glyff's
``YieldException``), so the rest of sefia treats them as recoverable: a transient
provider failure is never engraved as a permanent failure, and the run stays
resumable. They live here, in the adapter, because the exact set of transport
