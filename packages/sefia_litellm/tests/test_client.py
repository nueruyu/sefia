...
class TestLiteLLMClient:
    ...
    async def test_transient_provider_errors_become_recoverable(
        self, mock_acompletion, provider_error
    ):
        mock_acompletion.side_effect = provider_error
        client = LiteLLMClient(model="gpt-4o")

        with pytest.raises(RecoverableInferenceError):
            await client.complete([])

    async def test_non_transient_provider_error_propagates(self, mock_acompletion):
        # A deterministic error (bad credentials) must surface as itself so it is
        # engraved as a genuine failure, not masked as a recoverable one.
