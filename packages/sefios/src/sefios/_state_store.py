...
class StateStore(Generic[T]):
    """
    Provides a type-safe, caching wrapper around a SessionStorage for a specific key and state type.
    """

    def __init__(self, storage: SessionStorage, key: str, state_type: Type[T]):
    ...
    async def ensure(self) -> T:
        ...
        state = await self._storage.get(self._key, self._state_type)
        if state is None:
    ...
    async def get(self, default: T | None = None) -> T | None:
        ...
        self._cache = await self._storage.get(self._key, self._state_type)
        self._is_loaded = True
        return self._cache if self._cache is not None else default
    ...
    async def save(self, state: T) -> None:
        ...
        await self._storage.set(self._key, state, self._state_type)
        self._cache = state
        self._is_loaded = True
